...
(function() {
    ...
    // Navigate via currentScript — works 100% of the time, no ID lookup needed
    var script = document.currentScript;
    var wrap = script.previousElementSibling;
    // wrap is the ticker div; the text div is its last child
    var el = wrap ? wrap.querySelector('div[style]') : null;
...
if page == "🏠  Home":
    ...
    (function() {
        ...
        // Navigate via currentScript — no ID lookup, guaranteed to find siblings
        var script = document.currentScript;
        var row = script.previousElementSibling;
        if (row) {
            var nums = row.querySelectorAll('.stat-number');
            if (nums.length === 4) {
                animateCounter(nums[0], { end: 15,  suffix: 'B',  duration: 1500 });
                animateCounter(nums[1], { end: 2.6, decimals: 1, suffix: 'T',  duration: 1700 });
                animateCounter(nums[2], { end: 2,   prefix: '$', suffix: 'B+', duration: 1400 });
                animateCounter(nums[3], { end: 30,  suffix: '%', duration: 1600 });
            }
        }
    })();
    </script>
